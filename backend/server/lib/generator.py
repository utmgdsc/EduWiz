...
def ask(prompt):
    ...
    animator_system_message = SystemMessagePromptTemplate.from_template("You will be provided a script for a video. Write a manim script to animate this. Make sure that you STRICTLY use ascii characters only. Make sure that it renders properly and none of the elements of the video overlap. Make sure that the code runs and does not cause any errors. Use Write() for text, Create() for shapes, and Transform() for smooth transitions. Follow the script's order carefully to ensure proper animations. Use .next_to(), .move_to(), and .align_to() to position elements correctly. Avoid overlapping elements. The output should be a valid and runnable Python script using Manim.")
    animator_prompt = ChatPromptTemplate([animator_system_message, animator_human_message])

    chain = writer_prompt | writer | RunnableLambda(animatorPrompter) | animator_prompt | animator | RunnableLambda(codeCleaner)
