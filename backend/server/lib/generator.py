...
def ask(prompt):
    ...
    writer_system_message = SystemMessagePromptTemplate.from_template(
        "You will be provided a question, your job is to write a script for a video that will explain the answer to that question. The video will be coded via manim, so describe video elements as well. Describe each step clearly. Specify the animation order (e.g., first draw the triangle, then label the sides, then show the theorem). Avoid overlapping elements by specifying where each object should appear. Mention the type of animations (e.g., FadeIn, Write, Transform). Keep it concise and suitable for automatic conversion into Python code."
    )
    ...
    animator_system_message = SystemMessagePromptTemplate.from_template(
        "You will be provided a script for a video. Write a manim script to animate this. Make sure that you STRICTLY use ascii characters in the code. Make sure that it renders properly and none of the elements of the video overlap. Make sure that the code runs and does not cause any errors. Use Write() for text, Create() for shapes, and Transform() for smooth transitions. Follow the script's order carefully to ensure proper animations. Use .next_to(), .move_to(), and .align_to() to position elements correctly. Avoid overlapping elements. The output should be a valid and runnable Python script using Manim."
    )
    ...
    checker_system_message = SystemMessagePromptTemplate.from_template(
        "You are a manim expert. Your job is to check the following code for any errors and return the verified code. Make sure you return nothing except the code."
    )
    ...
    chain = (
        writer_prompt
        | writer
        | RunnableLambda(animatorPrompter)
        | animator_prompt
        | animator
